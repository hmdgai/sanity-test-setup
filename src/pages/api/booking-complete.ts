 ...
 * ─────────────────────────────────────────────────────────────────
 * Server-side relay: receives a booking_completed event from the
...
interface BookingCompletePayload {
  client_id:    string;
  session_id?:  string;
  booking_url?: string;
  page_url?:    string;
...
export async function POST({ request }: { request: Request }): Promise<Response> {
  ...
  // ── 4. Parse payload ─────────────────────────────────────────────
  let payload: BookingCompletePayload;
  try {
  ...
  if (!apiSecret || !measurementId) {
    console.warn('[HMDG CCM] booking-complete: GA4_API_SECRET or GA4_MEASUREMENT_ID not set');
  ...
  try {
    ...
    console.error('[HMDG CCM] booking-complete: GA4 MP returned', mpResponse.status);
    return new Response(JSON.stringify({ ok: false }), { status: 200 });
  } catch (err) {
    console.error('[HMDG CCM] booking-complete: fetch to GA4 failed', err);
    return new Response(JSON.stringify({ ok: false }), { status: 200 });
  }
}
